...
<\body>
  ...
  <\itemize-dot>
    ...
    is a typed forest>, G<text| is a subforest of <math|F>>|}>>.

    <item>Denote by <math|Vec<around*|(|<with|font|cal*|P>|)>\<subseteq\>\<bbb-R\><rsup|<with|font|cal*|P>>>
    the free vector space generated by <math|<with|font|cal*|P>>.

    <item>For all <math|<around*|(|G;F|)>\<in\><with|font|cal*|P>> we are
    given a (finite) collection <math|\<frak-A\><around*|(|G;F|)>> of
    subforests <math|A> of <math|F> s.t. <math|G\<subseteq\>A\<subseteq\>F>.

    <item>Define <math|\<Delta\>:Vec<around*|(|<with|font|cal*|P>|)>\<longrightarrow\>Vec<around*|(|<with|font|cal*|P>|)>\<otimes\>Vec<around*|(|<with|font|cal*|P>|)>>
    by
